...
// Fetch all arbitrators using direct RPC calls to isArbitrator
// Strategy: scan known addresses + use totalMinted to bound the search
export function useAllArbitrators(): string[] {
  const { data } = useReadContract({
    address: CONTRACTS.ARBITRATOR_NFT,
    abi: ARBITRATOR_NFT_ABI,
    functionName: "getArbitrators",
    query: { staleTime: 30000, retry: 1 },
  })
